...
process LOAD_DENSE_DEPTH {
    ...
    echo "curl -s -o response.json -w '%{http_code}' --retry 3 --retry-delay 5 --retry-connrefused --max-time 600 -X POST ${serviceUrl}/variants-db/load-dense-depth -H 'Content-Type: application/json' -H 'x-api-key: ***' -d '{\"fileUrl\": \"${gcsPath}\", \"sample_id\": \"${sampleId}\"}'"
